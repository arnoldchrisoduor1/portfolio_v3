...
const blogPosts: BlogPost[] = [
  {
    ...
    description:
      "Exploring cutting-edge methods for optimizing neural network training, including transfer learning, hyperparameter tuning, and regularization techniques.",
  ...
  },
  {
    ...
    description:
      "Leveraging Elixir's concurrency model and Phoenix framework to build scalable, real-time APIs that handle millions of requests.",
  ...
  },
  {
    ...
    description:
      "Comprehensive guide to building robust, secure, and scalable REST APIs using Django REST Framework with best practices.",
  ...
  },
  {
    ...
    description:
      "Deep dive into the mathematics and implementation of diffusion models for image generation, including Stable Diffusion architecture.",
  ...
  },
  {
    ...
    description:
      "Exploring the evolution of LLMs, transformer architecture, and practical implementation strategies for real-world applications.",
...
const researchProjects: ResearchProject[] = [
  {
    ...
    description:
      "A novel authentication system combining blockchain technology with neural networks for secure, decentralized biometric authentication.",
    ...
    methodology:
      "The research employed a mixed-methods approach combining quantitative analysis of authentication accuracy with qualitative assessment of system security and usability.",
    results:
      "The AuthoRyse system achieved 99.2% authentication accuracy while reducing vulnerability to common attacks by 87% compared to traditional systems.",
...
const ResearchPage: React.FC = () => {
  ...
  return (
    <>
      ...
      <main className="min-h-screen pt-20">
        ...
        <section className="py-8 px-4 sm:px-6 lg:px-8">
          <div className="max-w-6xl mx-auto">
            <motion.div className="flex justify-center gap-4 mb-12" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }}>
              <button
                onClick={() => setActiveSection("blog")}
                className={`px-8 py-4 rounded-full border text-lg font-medium transition-all duration-300 flex items-center gap-3 ${
                  activeSection === "blog" ? "bg-green-500 text-white border-green-500" : "bg-white/5 border-white/10 text-gray-300 hover:bg-white/10 hover:border-white/20"
                }`}
              >
                <BookOpen size={20} />
                Technical Blog
              ...
              <button
                ...
                className={`px-8 py-4 rounded-full border text-lg font-medium transition-all duration-300 flex items-center gap-3 ${
                  activeSection === "research" ? "bg-green-500 text-white border-green-500" : "bg-white/5 border-white/10 text-gray-300 hover:bg-white/10 hover:border-white/20"
                }`}
              >
                <FileText size={20} />
                Research Projects
            ...
            <AnimatePresence mode="wait">
              {activeSection === "blog" && (
                <motion.div key="blog" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.5 }}>
                  <BlogSection posts={blogPosts} />
                </motion.div>
              )}

              {activeSection === "research" && (
                <motion.div key="research" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.5 }}>
                  <ResearchSection projects={researchProjects} onProjectSelect={openResearchModal} />
                </motion.div>
              )}
            </AnimatePresence>
          </div>
      ...
      <AnimatePresence>{selectedResearch && <ResearchModal project={selectedResearch} onClose={closeResearchModal} />}</AnimatePresence>
  ...
  return (
    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
      {posts.map((post, index) => (
        ...
        >
          <a href={post.link} target="_blank" rel="noopener noreferrer" className="block bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl overflow-hidden hover:bg-white/10 transition-all duration-300 group-hover:scale-105 group-hover:border-white/20">
            {/* Blog Image */}
            <div className="relative h-48 overflow-hidden">
              ...
              <div className="absolute top-4 left-4">
                <span className="px-3 py-1 bg-green-500/80 backdrop-blur-sm rounded-full text-xs text-white font-medium">{post.category}</span>
              </div>
            </div>
            ...
            <div className="p-6">
              ...
              <h3 className="text-xl font-semibold text-white mb-3 line-clamp-2 group-hover:text-green-400 transition-colors">{post.title}</h3>
              ...
              <div className="flex items-center justify-between">
                <span className="text-green-400 text-sm font-medium group-hover:underline">Read on Medium →</span>
...
const ResearchSection: React.FC<{ projects: ResearchProject[]; onProjectSelect: (p: ResearchProject) => void }> = ({ projects, onProjectSelect }) => {
  return (
              ...
              <div className="flex flex-col lg:flex-row gap-8">
                ...
                <div className="lg:w-2/3 space-y-4">
                  <div className="flex items-center gap-3 mb-4">
                    <span className="px-3 py-1 bg-green-500/20 text-green-400 rounded-full text-sm font-medium">{project.category}</span>
                    <span className="px-3 py-1 bg-blue-500/20 text-blue-400 rounded-full text-sm font-medium">{project.status}</span>
                    <span className="text-gray-400 text-sm">{project.date}</span>
                  </div>

                  <h3 className="text-2xl font-bold text-white group-hover:text-green-400 transition-colors">{project.title}</h3>
                  ...
                  <div className="flex items-center justify-between pt-6">
                    <span className="text-green-400 text-sm font-medium group-hover:underline">View Research Details →</span>
...
const ResearchModal: React.FC<{ project: ResearchProject; onClose: () => void }> = ({ project, onClose }) => {
  return (
        ...
        <div className="relative">
          ...
          <div className="p-6 space-y-8">
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-6 border-t border-green-500/20">
              {project.link && (
                <a href={project.link} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-6 py-3 bg-green-500 hover:bg-green-600 rounded-lg transition-colors text-white font-medium">
